...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
    ...
    $biblio_q = $dbs->query('SELECT IF(i.call_number<>\'\', i.call_number, b.call_number), i.item_code, b.classification, i.location_id FROM biblio AS b LEFT JOIN item AS i ON b.biblio_id=i.biblio_id WHERE ' . $criteria);
   ...
   while ($biblio_d = $biblio_q->fetch_row()) {
    ...
    foreach ($chunked_label_arrays as $label_data) {
        ...
        foreach ($label_data as $labels) {
            ...
            $html_str .= '<div class="card card-body"><div class="align-items-center">';
            $html_str .= '<div>';
			...
			$sliced_label_1 = preg_split("/((?<=\w)\s+(?=\D))|((?<=\D)\s+(?=\d))/m", $label_1);
			$sliced_label_2 = preg_split("/((?<=\w)\s+(?=\D))|((?<=\D)\s+(?=\d))/m", $label_2);
            $html_str .= '<div class="labelStyle">';
            foreach ($sliced_label as $slice_label_item) {
                $html_str .= $slice_label_item . '/';
            }
			foreach ($sliced_label_1 as $slice_label_item_1) {
                $html_str .= $slice_label_item_1 . '<br />';
            }
			foreach ($sliced_label_2 as $slice_label_item_2) {
                $html_str .= $slice_label_item_2 . '<br />';
            }
			$html_str .= '<img class="img-fluid" height="150"  src="' . SWB . IMG . '/barcodes/' . urlencode(urlencode($barcode_text)) . '.png?' . date('YmdHis') . '" border="0" />';
